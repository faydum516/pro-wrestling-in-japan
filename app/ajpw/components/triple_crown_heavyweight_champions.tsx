...
import getAllChampions from "../lib/triple_crown_heavyweight_champions/getAllChampions";
import getAllChampionsByReign from "../lib/triple_crown_heavyweight_champions/getAllChampionsByReign";
import getLongestReigningChampion from "../lib/triple_crown_heavyweight_champions/getLongestReigningChampion";
import getChampionWithMostReigns from "../lib/triple_crown_heavyweight_champions/getChampionWithMostReigns";
import getYoungestChampion from '../lib/triple_crown_heavyweight_champions/getYoungestChampion';
import getOldestChampion from '../lib/triple_crown_heavyweight_champions/getOldestChampion';
import getChampionWithMostDefenses from '../lib/triple_crown_heavyweight_champions/getChampionWithMostDefenses';
import getChampionWithMostCombinedDefenses from '../lib/triple_crown_heavyweight_champions/getChampionWithMostCombinedDefenses';

// Adding images
import Image from 'next/image';
import belt from '../img/triple_crown_heavyweight_championship.jpg';
import pie_chart from '../img/Triple Crown Heavyweight Champions.png';

export default async function TripleCrownHeavyweightChampions() {
    ...
    const mostDefenses = Array.from(new Set<number>(championWithMostDefenses.map((champ) => champ.Defenses)));

    // Championship Belt
    const championship = "Triple Crown Heavyweight Championship";
        ...
        <section className="flex justify-center items-center flex-col">
            <div className="mb-6 flex justify-center items-center flex-col">
                ...
                <p className="w-4/5 pt-3 pb-4 phone:text-sm">
                    There have been {championsByReign.filter((champ) => champ.Champion !== "Vacated" && champ.Champion !== "Unified").length} championship reigns so far with {champions.length} wrestlers in total having won the title. {currentChampion} The late great {longestReigningChampion[0].Champion}, who claimed the championship {championsByReign.filter((champ) => champ.Champion == longestReigningChampion[0].Champion)[championsByReign.filter((champ) => champ.Champion == longestReigningChampion[0].Champion).length - 1].Reign} times, still holds the records for the longest reign at {longestReigningChampion[0].LongestDays} days and the longest combined reign at {champions[0].CombinedDays} days. {championWithMostReigns[0].Champion} currently holds the record for most reigns with {championWithMostReigns[0].MostReigns}. {youngestChampion[0].Champion} is currently the youngest champion at {youngestAge} old, while {oldestChampion[0].Champion} is currently the oldest champion at {oldestAge}. {championWithMostCombinedDefenses[0].Champion.split(' ')[1]} holds the record for most combined defenses at {championWithMostCombinedDefenses[0].SumOfDefenses}, while he&apos;s tied with {championWithMostDefenses[0].Champion} for the most successful defenses in a single reign at {mostDefenses[0]}.
                </p>  
                <figure>
                    <Image     
                        src={belt}
            ...
            <div className="mb-6">
                ...
                <div className="flex scroll phone:w-[325px] overflow-x-auto">
                    <table className="border border-black font-work_sans border-solid border-collapse text-center font-worksans min-w-[450px] mb-1">
                        <thead>
                            <tr className="bg-[#222225] text-white phone:text-sm">
                                <th className="border border-black border-solid border-collapse w-14 py-2">No.</th>
                                <th className="border border-black border-solid border-collapse w-44 py-2">Champion</th>
            ...
            <p className="w-4/5 pt-3 pb-4 phone:text-sm">Additionally, we conducted an analysis using a pie chart to determine the number of wrestlers who won the Triple Crown Heavyweight Championship for the first time in each respective age group by decade.</p>
            <div className="flex scroll phone:w-[325px] phone:mb-5 overflow-x-auto">
                <figure className="pt-4 pb-5">
                    <Image     
                        ...
                        width={0}
                        height={0}
                        alt="Triple Crown Heavyweight Champions"
                        className="w-[650px] h-auto phone:w-[450px]"
                        priority
                    />  
                    <figcaption className="text-center w-[650px] phone:w-[400px] phone:text-sm">Pie Chart of all the Triple Crown Heavyweight Champions &#40;in Age Groups by Decade&#41;</figcaption>
                </figure>
            </div>
            <p className="w-4/5 pt-3 pb-10 phone:text-sm">As we can see in the pie chart above, 4 wrestlers won the AJPW heavyweight title in their 20s, 17 in their 30s, 9 in their 40s, and only 1 in their 50s or over. Most of the wrestlers won the world title in their 30s likely because most of them started their wrestling careers in their late teens or early 20s. World championships are hard to achieve, so wrestlers have to wrestle hard in the ring in order to have a chance to claim them.</p>
        </section>
    );
